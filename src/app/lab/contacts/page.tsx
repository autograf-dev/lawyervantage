...
export default function Page() {
  ...
  async function handleCreateSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      ...
      try {
        const res = await fetch(`https://lawyervantage-legallab.netlify.app/.netlify/functions/updateCustomer?id=${encodeURIComponent(editingId)}`, {
          method: "PUT",
    ...
    const payload = {
      ...
      phone,
      optionalFields: {
        companyName: "Lawyer Vantage Legal Lab",
        tags: ["new", "lead"],
      },
    }
    // optimistic add
    ...
    try {
      const res = await fetch("https://lawyervantage-legallab.netlify.app/.netlify/functions/addContact", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      })
  ...
  async function handleDeleteConfirmed() {
    ...
    try {
      const res = await fetch(`https://lawyervantage-legallab.netlify.app/.netlify/functions/deleteContact?id=${encodeURIComponent(id)}`)
      ...
      <SidebarProvider>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-[1.4rem] font-semibold leading-none">Contacts - Legal Lab</h1>
