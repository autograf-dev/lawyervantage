    ...
    async function fetchOpportunities() {
      ...
      try {
        ...
        const mapped: Opportunity[] = arr.map((o) => ({
          ...
          source: String(o.source ?? "Lawyer Vantage Tc Legal"),
...
export default function Page() {
  ...
  async function handleCreateSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      ...
      const optimisticUpdated: Opportunity = {
        ...
        contactId,
        status,
        source: previous?.source || "Lawyer Vantage Tc Legal",
        updatedAt: now,
        contact: {
      ...
      try {
        const payload: { id: string; name: string; status: string; monetaryValue: number; source: string; contactId?: string } = { id: editingId, name, status, monetaryValue, source: "Lawyer Vantage Tc Legal" }
        if (contactId && contactId !== previous?.contactId) payload.contactId = contactId
        const res = await fetch("https://lawyervantage-tclegal.netlify.app/.netlify/functions/updateOpportunity", {
          method: "PUT",
    ...
    const optimistic: Opportunity = {
      ...
      assignedTo: mkId(),
      status,
      source: "Lawyer Vantage Tc Legal",
    ...
    try {
      const res = await fetch("https://lawyervantage-tclegal.netlify.app/.netlify/functions/addOpportunity", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, contactId, monetaryValue, status, source: "Lawyer Vantage Tc Legal" }),
      })
  ...
  async function handleDeleteConfirmed() {
    ...
    try {
      const res = await fetch("https://lawyervantage-tclegal.netlify.app/.netlify/functions/deleteOpportunity", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      <SidebarProvider>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-[1.4rem] font-semibold leading-none">Opportunities - Tc Legal</h1>
